...
void executeWebSocketMessage(AsyncWebSocketClient* client, uint8_t* data, size_t len) {
	...
	switch (msgCode)
	{
		case 1:
			...
			{
				setWebSocketContent((uint8_t*)&DIN_Values[i], 1);
			}
			for (int i = 0; i < NUM_DISCRETE_OUTPUT; i++)
			{
				setWebSocketContent((uint8_t*)&DOUT_Values[i], 1);
			}
			for (int i = 0; i < NUM_ANALOG_INPUT; i++)
			{
				setWebSocketContent((uint8_t*)&AIN_Values[i], 2);
			}
			for (int i = 0; i < NUM_ANALOG_OUTPUT; i++)
			{
				setWebSocketContent((uint8_t*)&AOUT_Values[i], 2);
			}
